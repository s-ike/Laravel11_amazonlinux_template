...
init: ## 初期処理を行います。開発環境の作成を行います。
	...
	${DOCKER_CMD} build
	@make up
	${WEB_CMD} composer install
	${WEB_CMD} php artisan key:generate
	${WEB_CMD} php artisan storage:link
	${WEB_CMD} chmod -R 777 storage bootstrap/cache
	@make fresh

.PHONY: cache
cache: ## Laravelのキャッシュを作成します。
	${WEB_CMD} composer dump-autoload -o
	${WEB_CMD} php artisan optimize

.PHONY: clear
clear: ## Laravelのキャッシュクリアを実行します。
	${WEB_CMD} composer clear-cache
	${WEB_CMD} php artisan optimize:clear
...
migrate: ## データベースの更新を行います。
	${WEB_CMD} php artisan migrate

.PHONY:	rollback
rollback: ## データベースのロールバックを行います。
	${WEB_CMD} php artisan migrate:rollback

.PHONY:	fresh
fresh: ## データベースをリセットします。
	${WEB_CMD} php artisan migrate:fresh --seed

.PHONY:	refresh
refresh: ## データベースをrefreshによりリセットします。
	${WEB_CMD} php artisan migrate:refresh --seed

.PHONY:	seed
seed: ## データベースへシードを実行します。
	${WEB_CMD} php artisan db:seed

.PHONY:	tinker
tinker: ## tinkerを実行します。
	${WEB_CMD} php artisan tinker
